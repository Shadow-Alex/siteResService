...
type Order struct {
	...
	GoodId     uint   `orm:"column(good_id)"`
	FinalPrice uint   `orm:"column(final_price);null"`
	UserId     uint   `orm:"column(user_id)"`
	AddressId  int    `orm:"column(address_id)"`
	OrderSn    string `orm:"column(order_sn);size(64);null"`
	UtmSource  string `orm:"column(utm_source);size(255);null"`
	Count      int    `orm:"column(count);null"`
	Channel    string `orm:"column(channel);size(255);null" description:"物流方式(宅配/店配)"`
